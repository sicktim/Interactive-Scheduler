...
**Source file:** `Interactive-scheduler/interactive-scheduler.html` (v4.1.0, 9054 lines)
...
- All components prefixed `Whiteboard*` (lines 6193–8034)
- The `HighlightPicker` component (lines 6169–6190)
- The `PlaceholderChip` component (lines 6375–6580)
- The `WhiteboardCrewGroup` component (lines 6585–6819)
- The `WhiteboardCrewCell` component (lines 6266–6366)
- The `wbRowClass` helper (lines 6821–6826)
- The `PendingTimeInput` component (lines 6832–6867, used by `WhiteboardSupervision` for pending slot time fields)
- The `DutyPuckChip` and `DutyDropZone` inner components (lines 7705–7730, 7733–7762, defined inside `WhiteboardView`)
- The `DUTY_STORAGE_KEY` localStorage key (`tps-duty-assignments`, line 7635)
- The `HIGHLIGHT_STORAGE_KEY` localStorage key (`tps-scheduler-highlights`, line 3871)
...
- Light-mode overrides for all of the above (lines 2299–2358)
- Constants: `WB_SECTION_ORDER`, `SUPV_DUTY_ORDER`, `ACAD_FIXED_CATS`, `WB_HIGHLIGHT_COLORS`, `WB_HIGHLIGHT_BG` (lines 2500–2614)
- Placeholder system constants: `BLANK_PUCK_DEFS`, `RENAMEABLE_PH_ROLES`, `PLACEHOLDER_FILL_RULES`, `PLACEHOLDER_ROLE_OPTIONS`, `GENERIC_PUCK_COLOR`, `canFillPlaceholder`, `getPlaceholderRoleColor`, `makePlaceholder` (lines 2541–2603)
- `handleCreateWbEvent` wrapper (line 7857) and the "Add" buttons inside each sub-table
- `foaAuthBadges` useMemo (line 7882)
- Scroll position persistence per day (lines 7764–7780)
- `wbActiveDay` state in `SchedulerView` (line 8044)
- The `viewMode === 'whiteboard'` branch and its `display:none` wrapper (line 8697)
...
|---|---|---|
| `WhiteboardView` | 7623–8034 | Root component; owns day state, highlights, duty pucks, scroll position |
| `WhiteboardSupervision` | 6870–7210 | Duty-grouped table (SOF, OS, ODO, FDOs); pending empty triplets with `PendingTimeInput`/`handlePocDrop`; up to 4 POC/Start/End triplets per row |
| `WhiteboardFlying` | 7213–7348 | Flying events table; 12 columns including Model, Brief, ETD, ETA, Debrief, Crew |
| `WhiteboardGround` | 7351–7468 | Ground events table; 9 columns including Event, Start, End, Person(s) |
| `WhiteboardAcademics` | 7471–7518 | Read-only academics sidebar; always renders 4 fixed rows |
| `WhiteboardNA` | 7521–7620 | Non-availability table; no status checkboxes |
| `WhiteboardCrewGroup` | 6585–6819 | Multi-person crew cell with flex-wrap chips, placeholders, and + menu |
| `WhiteboardCrewCell` | 6266–6366 | Single-occupancy crew cell (used only by Supervision POC column on existing events) |
| `PlaceholderChip` | 6375–6580 | Dashed unfilled slot chip with drag-fill, rename, and warning prompt |
| `WhiteboardCell` | 6193–6264 | Inline-editable `<td>`; supports text and time (HH:MM) fields; Tab/Shift+Tab navigation |
| `HighlightPicker` | 6169–6190 | `position:fixed` color-swatch popup portal; appears near clicked cell |
| `PendingTimeInput` | 6832–6867 | Lightweight time input for pending supervision triplets; manages its own draft state; fires `onCommit` on blur/Enter |
| `DutyPuckChip` | 7705–7730 | Colored chip in FOA/AUTH header slots; draggable |
| `DutyDropZone` | 7733–7762 | Drop target "+"-styled zone in FOA/AUTH header |
| `wbRowClass` | 6821–6826 | Returns `wb-row-effective`, `wb-row-partial`, or `wb-row-cancelled` per event status |

### Supporting constants (defined at module scope)

| Constant | Line | Value / Purpose |
|---|---|---|
| `WB_SECTION_ORDER` | 2500 | `['Supervision', 'Flying', 'Ground', 'NA']` — render order |
| `SUPV_DUTY_ORDER` | 2501 | `['SOF', 'OS', 'ODO', 'F-16 FDO', 'T-38 TDO', 'C-12 TDO', 'A-29 ADO']` — no "Other (As Req'd)"; custom duties create dynamic rows |
| `ACAD_FIXED_CATS` | 2503–2510 | 4-element array for fixed Academics rows (Alpha/Bravo x FTC/STC) |
| `WB_HIGHLIGHT_COLORS` | 2605–2613 | 7 swatch definitions (yellow, ftca, ftcb, stca, stcb, ip, ifte) |
| `PLACEHOLDER_ROLE_OPTIONS` | 2593–2603 | Ordered list for the add-placeholder popover |
| `PLACEHOLDER_FILL_RULES` | 2560–2571 | Which roster categories can fill each placeholder role |
| `RENAMEABLE_PH_ROLES` | 2554 | `Set(['IP', 'IFTE/ICSO', 'Staff STC', 'Generic'])` |
| `GENERIC_PUCK_COLOR` | 2541 | `{ bg: '#f8fafc', text: '#1e293b' }` — white puck for generic role |
| `BLANK_PUCK_DEFS` | 2543–2552 | 7 blank-puck definitions for picker (IP, IFTE/ICSO, FTC-A, FTC-B, STC-A, STC-B, Generic) |
| `TIME_RE` | 2511 | `/^([01]\d|2[0-3]):[0-5]\d$/` — military time validation |
| `DUTY_STORAGE_KEY` | 7635 | `'tps-duty-assignments'` |
| `HIGHLIGHT_STORAGE_KEY` | 3871 | `'tps-scheduler-highlights'` |

---

## Table Structure

### Supervision Table (WhiteboardSupervision, lines 6870–7210)

Uses `.wb-table.wb-table-compact`. The table has a fixed structure regardless of how many events exist for a given duty role.

**Column layout (colgroup, lines 6940–6953):**

```
Duty (7.5%) | [POC (6%) | Start (4.5%) | End (4.5%)] × 4 | Notes (flex) | Delete (20px)
```

The four POC/Start/End triplets (`maxTriplets = 4`, line 6936) correspond to up to four supervision events under the same duty type on a given day (e.g., SOF Alpha morning + SOF Bravo afternoon).

**Header row (lines 6955–6968):** `Duty | POC | Start | End | POC | Start | End | POC | Start | End | POC | Start | End | Notes | (blank)`
...
**Empty triplets (pending slots):** If `evts[i]` is null, the slot renders three `<td>` elements containing a `PendingTimeInput` for Start, a `PendingTimeInput` for End, and a POC `<td>` that is a drop target (`handlePocDrop`). The state for in-progress (not-yet-created) triplets is held in `pendingSlots` (a `useState` object keyed by `slotKey = "${duty}:${i}"`). Once both times are valid AND a POC has been dropped, `onCreateEvent` fires immediately and the `pendingSlots` entry is removed. If only the POC is dropped first (times absent), the POC is stored in `pendingSlots[slotKey].poc` and `pocSourceId` for deferred creation when times are later entered. `PendingTimeInput` renders an amber-bordered input when the field has content that fails `TIME_RE` validation.
...
### Flying Table (WhiteboardFlying, lines 7213–7348)
...
### Ground Table (WhiteboardGround, lines 7351–7468)
...
### NA Table (WhiteboardNA, lines 7521–7620)
...
### Academics Sidebar (WhiteboardAcademics, lines 7471–7518)
...
## WhiteboardCell Component (lines 6193–6264)
...
## WhiteboardCrewCell Component (lines 6266–6366)
...
## WhiteboardCrewGroup Component (lines 6585–6819)
...
## PlaceholderChip Component (lines 6375–6580)
...
`WhiteboardSupervision` (lines 6870–7210) receives all `section === 'Supervision'` events for the active day and groups them by duty role using `byDuty` (a `useMemo`).

**`pendingSlots` state:** A `useState({})` object (line 6898) keyed by `"${duty}:${i}"` (e.g., `"SOF:2"`). Each entry holds `{ startTime, endTime, poc, pocSourceId }` for an in-progress empty triplet that has not yet been committed as a real event. `PendingTimeInput` components write into this object via `handleTimeCommit`; `handlePocDrop` writes `poc` and `pocSourceId`. When all three fields (start, end, poc) are valid, `onCreateEvent` fires and the entry is deleted. The `pendingSlots` state is reset on `activeDay` change (handled by the `PendingTimeInput` internal `useEffect` that tracks the `value` prop).
...
**`wb-row-focused` CSS (lines 1735–1748):**
...
**`wb-row-dimmed` CSS (lines 1749–1751):**
- `opacity: 0.35`
...
**Hover override (lines 1729–1732):** The base `.wb-table tr:hover td` rule sets a faint background. Status rows use `!important` to maintain their tinted color on hover (e.g., `rgba(34,197,94,0.18)` for effective rows). Without `!important` the hover rule would overwrite the status color.
...
| `SECTION_ORDER` vs `WB_SECTION_ORDER` | Distinct | `SECTION_ORDER` (line 2394) is for the Timeline/Selection screen; `WB_SECTION_ORDER` (line 2398) includes Supervision and is used exclusively by Whiteboard |
| `clearWorkingCopy()` | Shared | On "Refresh from Whiteboard", also clears `tps-scheduler-highlights` (HIGHLIGHT_STORAGE_KEY, line 3871) |
...
User feedback (line 215): "There is no way to add/edit a new supervision column group ... if a supervision event is created on the timeline view, it will show as a new line in Whiteboard." This was partially addressed in v4.1.0 T7 — empty supervision triplets now show `PendingTimeInput` fields and a droppable POC cell. Time entry (via `handleTimeCommit`) combined with a POC drop (`handlePocDrop`) now triggers event creation inline, without a button.
...
### Modifying `WhiteboardView` (lines 7623–8034)
...
### Modifying `WhiteboardSupervision` (lines 6870–7210)
...
- [ ] Does `pendingSlots` reset properly when `activeDay` changes? (The `PendingTimeInput` `useEffect` on its `value` prop handles draft reset; parent should clear `pendingSlots` on day change or pass empty slot objects.)
...
### Modifying `WhiteboardCell` (lines 6193–6264)
...
### Modifying `WhiteboardCrewGroup` (lines 6585–6819)
...
### Modifying `PlaceholderChip` (lines 6375–6580)
...
1. Add the section name to `WB_SECTION_ORDER` (line 2500)
2. Create a new `Whiteboard{Section}` component following the same pattern as `WhiteboardFlying`
3. Add the section's render branch inside `WhiteboardView`'s `WB_SECTION_ORDER.map(sec => ...)` (near line 7948)
...
5. Add a section to `SECTION_ORDER` (near line 2394–2398) if the new section should also appear in the Timeline picker
