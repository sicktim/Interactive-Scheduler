/**
 * MCG 26A V1 → V2 Transformation Script
 *
 ...
 *
 * Usage: node transform-v2.cjs
 *
 * All 9 phases: AN, AS, CF, FQ, PF, SO, SY, TF, TL
 */
...
const CREW_SOLO_MAP = {
  ...
  'CF 6551F': 'C-12',  // C-12 Flight Training (non-crew-solo pilot)
...
function expandApplicability(appArray) {
  ...
  for (const tag of appArray) {
    ...
    // Handle compound tags like "crew solo", "non-crew solo", "select P" — skip silently
  }
...
function parsePrereqNotes(notes, prereqCode, eventApplicability) {
  ...
  // ── Step 0: [req'd for XX XXXXL] patterns — these are requiredFor scoping, not position scoping
  // The requiredFor field is already set in V1 data; this note is just for auditability
  if (/^\[?req'd for [A-Z]{2} \w+\]?$/.test(notes.trim())) {
    // Scope remains 'inherit' — this pattern only scopes WHICH event, not WHO
    return result;
  }

  // ── Step 0b: "TPS Enrollment" or similar enrollment notes
  if (/^TPS Enrollment$/i.test(notes.trim())) {
    result.errata = notes;
    return result;
  }
...
function main() {
  console.log('MCG 26A V1 → V2 Transformation');
  console.log('================================\n');
  ...
  // Check for unmatched notes
  for (const phase of allV2Phases) {
    for (const mod of phase.modules) {
      for (const ev of mod.events) {
        for (const prereq of ev.prerequisites) {
          // Skip [req'd for XX XXXXL] patterns — these are requiredFor scoping, handled correctly
          const isReqdForPattern = /^\[?req'd for [A-Z]{2} \w+\]?$/.test((prereq.originalNotes || '').trim());
